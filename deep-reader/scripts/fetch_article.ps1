#!/usr/bin/env pwsh
# =============================================================================
# 深度阅读文章抓取脚本（Windows PowerShell 版本）
# =============================================================================
# 为「没有 Git Bash、也没有 Python」的 Windows 用户提供的纯 PowerShell 实现。
# 仅依赖系统自带的 PowerShell（Windows PowerShell 5.1 或 PowerShell 7+），
# 无需 bash / curl / Python / 任何第三方包。
#
# 功能：伪装微信浏览器抓取 URL → HTML 清理 → 纯文本抽取 → 输出 JSON
# 输出与 fetch_article.py / .sh 一致：{"title","url","content","content_length"}
#   content        —— 纯文本（无 Markdown 转换，与 .sh fallback 一致）
#   content_length —— 中文字符数 + 英文词数（统计于截断前）
#
# 用法:
#   powershell -NoProfile -ExecutionPolicy Bypass -File fetch_article.ps1 <url>
# =============================================================================
...
function Get-ContentLength([string]$text) {
    ...
    $withoutCjk = [regex]::Replace($text, '[\u4e00-\u9fff\u3400-\u4dbf\uf900-\ufaff]', ' ')
    $en = 0
    foreach ($w in ($withoutCjk -split '\s+')) {
        if ($w -and ($w[0] -match '[A-Za-z0-9]')) { $en++ }
    }
    return $zh + $en
}
...
try {
    ...
    if ($wechat) {
        $fragment = $wechat
    } elseif ($html -match '(?is)<div[^>]*(?:id|class)=["''][^"'']*(?:rich_media_content|img-content|article-content|post-content)[^"'']*["''][^>]*>(.*)</div>') {
        $fragment = $matches[1]
    } elseif ($html -match '(?is)<article[^>]*>(.*)</article>') {
        $fragment = $matches[1]
    ...
    $contentLength = Get-ContentLength $fullText
